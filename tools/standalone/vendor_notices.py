...
def collect(app,evidence):
    ...
    if record.exists():
        ...
        if all((target/n).is_file() and digest(target/n)==h for n,h in old['notice_files'].items()):return
    ...
    for vendor,url,check,runtime in [('TensorRT',TRT,None,'nvinfer_11.dll'),('CUDA',CUDA,CUDA_SHA,'cudart64_13.dll')]:
        ...
        archive.unlink()
    # Preserve the vendor's full published SLA as well, not a paraphrase.
    sla=target/'TensorRT_LICENSE.html';fetch(SLA,sla)
    text=sla.read_text(encoding='utf-8')
    if 'TensorRT' not in text or 'redistribution' not in text.lower():raise RuntimeError('Invalid TensorRT SLA response')
    proof['notice_files'][sla.name]=digest(sla)
    record.write_text(json.dumps(proof,ensure_ascii=False,indent=2),encoding='utf-8')
    (evidence/'vendor-notices.json').write_text(record.read_text(encoding='utf-8'),encoding='utf-8')
