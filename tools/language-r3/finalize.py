...
# The original resource listed the log level "info" as a video driver. Match
# the frontend's actual default "gpu" and never write an invalid driver value.
...
extra={
 ...
 'always use software decoding':'始终使用软件解码',
 'enable best hw decoder':'自动选择适合的硬件解码器',
 'exactly the same as auto':'与 auto 相同',
 'enable best hw decoder with copy-back':'自动选择硬件解码器并复制回系统内存',
 ...
 'Static ONNX':'静态 ONNX','Static':'静态','Dynamic':'动态',
 'Engine Type':'引擎类型','none':'无','always':'始终','never':'从不','once':'一次',
 'yes (Default)':'启用（默认）','no (Default)':'关闭（默认）','auto (Default)':'自动（默认）'
}
...
p=Path('language-coverage.json');data=json.loads(p.read_text(encoding='utf-8'));data['strings']=len(data['translations']) if 'translations' in data else len(json.loads(Path('manager/AnimeJaNaiConfEditor/LanguageStrings.json').read_text(encoding='utf-8'))['translations']);data['display_only_default_profile_binding']=True;data['invalid_video_driver_removed']=True
p.write_text(json.dumps(data,ensure_ascii=False,indent=2),encoding='utf-8')
