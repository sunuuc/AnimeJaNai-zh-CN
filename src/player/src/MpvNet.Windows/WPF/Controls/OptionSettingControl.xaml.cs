...
public partial class OptionSettingControl : UserControl, ISettingControl
{
    ...
    public OptionSettingControl(OptionSetting optionSetting)
    {
        ...
        InitializeComponent();
        DataContext = this;
        TitleTextBox.Text = AnimeJaNai.Localization.UiText.T(optionSetting.Name) == optionSetting.Name ? optionSetting.Name : AnimeJaNai.Localization.UiText.T(optionSetting.Name) + " (" + optionSetting.Name + ")";

        if (string.IsNullOrEmpty(optionSetting.Help))
            HelpTextBox.Visibility = Visibility.Collapsed;

        HelpTextBox.Text = AnimeJaNai.Localization.UiText.T(optionSetting.Help);
